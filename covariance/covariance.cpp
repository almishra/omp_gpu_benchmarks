...
#pragma omp target exit data map(delete: X[0:N], Y[0:N])
  ...
  // CPU
  meanX = covariance_kernel1_cpu(X, fp);
  meanY = covariance_kernel1_cpu(Y, fp);
  cov = covariance_kernel2_cpu(X, Y, meanX, meanY, fp);
#ifdef DEBUG
  printf("Covariance (CPU)     = %lf\n", cov);
#endif

  // GPU MEMCPY
  meanX = covariance_kernel1_gpu_mem(X, fp);
  meanY = covariance_kernel1_gpu_mem(Y, fp);
  cov = covariance_kernel2_gpu_mem(X, Y, meanX, meanY, fp);
#ifdef DEBUG
  printf("Covariance (GPU_mem) = %lf\n", cov);
#endif

  // GPU
  meanX = covariance_kernel1_gpu(X, fp);
  meanY = covariance_kernel1_gpu(Y, fp);
  cov = covariance_kernel2_gpu(X, Y, meanX, meanY, fp);
#ifdef DEBUG
  printf("Covariance (GPU)     = %lf\n", cov);
#endif
