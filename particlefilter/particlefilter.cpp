...
void particleFilter(int *I, int IszX, int IszY, int Nfr, int * seed, FILE *fp) 
{
  ...
  //CPU execution
  particlefilter_kernel1_cpu(weights, arrayX, arrayY, xe, ye, fp);
  for(int k = 1; k < Nfr; k++) {
    init(arrayX, arrayY, weights, I, seed, countOnes, objxy, IszX, IszY, Nfr, k);
    double sumWeights = particlefilter_kernel2_cpu(weights, fp);
    particlefilter_kernel3_cpu(weights, sumWeights, fp);
    xe = 0; ye = 0;
    particlefilter_kernel4_cpu(arrayX, arrayY, weights, xe, ye, fp);
    resample(CDF, weights);
    particlefilter_kernel5_cpu(u, u1, fp);
    particlefilter_kernel6_cpu(CDF, u, arrayX, arrayY, xj, yj, fp);
    particlefilter_kernel7_cpu(weights, arrayX, arrayY, xj, yj, fp);
  }

  // GPU execution - With MEMCPY
  particlefilter_kernel1_gpu_mem(weights, arrayX, arrayY, xe, ye, fp);
  for(int k = 1; k < Nfr; k++) {
    init(arrayX, arrayY, weights, I, seed, countOnes, objxy, IszX, IszY, Nfr, k);
    double sumWeights = particlefilter_kernel2_gpu_mem(weights, fp);
    particlefilter_kernel3_gpu_mem(weights, sumWeights, fp);
    xe = 0; ye = 0;
    particlefilter_kernel4_gpu_mem(arrayX, arrayY, weights, xe, ye, fp);
    resample(CDF, weights);
    particlefilter_kernel5_gpu_mem(u, u1, fp);
    particlefilter_kernel6_gpu_mem(CDF, u, arrayX, arrayY, xj, yj, fp);
    particlefilter_kernel7_gpu_mem(weights, arrayX, arrayY, xj, yj, fp);
  }

  // GPU execution - Without MEMCPY
  particlefilter_kernel1_gpu(weights, arrayX, arrayY, xe, ye, fp);
  for(int k = 1; k < Nfr; k++) {
    init(arrayX, arrayY, weights, I, seed, countOnes, objxy, IszX, IszY, Nfr, k);
    double sumWeights = particlefilter_kernel2_gpu(weights, fp);
    particlefilter_kernel3_gpu(weights, sumWeights, fp);
    xe = 0; ye = 0;
    particlefilter_kernel4_gpu(arrayX, arrayY, weights, xe, ye, fp);
    resample(CDF, weights);
    particlefilter_kernel5_gpu(u, u1, fp);
    particlefilter_kernel6_gpu(CDF, u, arrayX, arrayY, xj, yj, fp);
    particlefilter_kernel7_gpu(weights, arrayX, arrayY, xj, yj, fp);
  }
}
