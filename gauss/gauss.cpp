...
#pragma omp target exit data map(delete: mat_gpu[0:N][0:N])                        
  ...
  cnt_iter = 0;
  while (!done && (cnt_iter < MAX_ITER)) {
    diff = kernel_cpu_collapse(mat_coll, fp);
    if (diff/mat_dim < TOL) {
      done = 1;
    }
    cnt_iter ++;
  ...
  cnt_iter = 0;
  while (!done && (cnt_iter < MAX_ITER)) {
    diff = kernel_gpu_mem(mat_gpu_mem, fp);
    if (diff/mat_dim < TOL) {
      done = 1;
    }
    cnt_iter ++;
  ...
  cnt_iter = 0;
  while (!done && (cnt_iter < MAX_ITER)) {
    diff = kernel_gpu_collapse_mem(mat_gpu_coll_mem, fp);
    if (diff/mat_dim < TOL) {
      done = 1;
    }
    cnt_iter ++;
...
#pragma omp target exit data map(delete: mat_gpu[0:N][0:N])
  ...
  cnt_iter = 0;
#pragma omp target enter data map(to: mat_gpu_coll[0:N][0:N])
  while (!done && (cnt_iter < MAX_ITER)) {
    diff = kernel_gpu_collapse(mat_gpu_coll, fp);
    if (diff/mat_dim < TOL) {
      done = 1;
    }
    cnt_iter ++;
  }
#pragma omp target exit data map(delete: mat_gpu_coll[0:N][0:N])
