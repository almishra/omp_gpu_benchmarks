...
#pragma omp target exit data map(delete: A[0:N1][0:N2], B[0:N2][0:N1])
  ...
  // CPU COLLAPSE
  transpose_kernel_cpu_collapse(A, B, fp);
#ifdef DEBUG
  printf("Matrix B (CPU Collapse)\n");
  for(int i=0; i<N2; i++) {
    for(int j=0; j<N1; j++) {
      printf("%2.2lf ", B[i][j]);
    }
  ...
  // GPU MEMCPY
  transpose_kernel_gpu_mem(A, B, fp);
#ifdef DEBUG
  printf("Matrix B (GPU - MEMCPY)\n");
  for(int i=0; i<N2; i++) {
    for(int j=0; j<N1; j++) {
      printf("%2.2lf ", B[i][j]);
    }
  ...
  // GPU COLLAPSE MEMCPY
  transpose_kernel_gpu_collapse_mem(A, B, fp);
#ifdef DEBUG
  printf("Matrix B (GPU - COLLAPSE MEMCPY)\n");
  for(int i=0; i<N2; i++) {
    for(int j=0; j<N1; j++) {
      printf("%2.2lf ", B[i][j]);
    }
  ...
  // GPU COLLAPSE
  transpose_kernel_gpu_collapse(A, B, fp);
#ifdef DEBUG
  printf("Matrix B (GPU)\n");
  for(int i=0; i<N2; i++) {
    for(int j=0; j<N1; j++) {
